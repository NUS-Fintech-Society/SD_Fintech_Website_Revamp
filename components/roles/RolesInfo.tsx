...
const RolesInfo = ({
  ...
  return (
    <>
      <div className="mb-10 mt-3 flex flex-wrap text-[24.48px] font-bold leading-8 text-[#002750]">
        <div className="relative h-[52px] w-[68.88px] sm:h-[139px] sm:w-[184px]">
          <Image src={logo.src} alt={logo.alt} layout="fill" />
        </div>
        <h1 className="ml-4 mt-3 underline underline-offset-[6px] sm:ml-6 sm:mt-10 sm:text-6xl sm:leading-[4.538rem] sm:underline-offset-[14px]">{`${name}`}</h1>
      </div>

      <RolesText header={aboutUs.header} desc={aboutUs.desc} />

      {subteams.length !== 0 && (
        <div className="mb-10 flex flex-col justify-center space-y-[30px] lg:flex-row lg:space-x-[70px] lg:space-y-0">
          {renderSubteamsCard}
        </div>
      )}
